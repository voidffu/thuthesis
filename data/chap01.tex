...
% RGB
%% 2DCNN
卷积神经网络（Convolutional Neural Network，CNN）已被证明是许多计算机视觉任务中的有效算法，它利用局部感受野、参数共享和降采样机制来提取图像特征。CNN主要由三类层次构成:卷积层负责特征提取，池化层实现特征压缩，全连接层完成特征映射。这种结构设计使CNN在计算机视觉领域获得了广泛应用。2DCNN方法被广泛用于处理静态手势图像的分类和识别任务。%这些网络通过卷积层和池化层有效地捕获图像中的空间特征。
%% Two stream
...
%% Recurrent
循环神经网络（Recurrent Neural Network，RNN）\cite{jozefowicz2015empirical}和长短期记忆网络（Long Short-Term Memory，LSTM）\cite{yue2015beyond}在序列数据处理中得到广泛应用。
